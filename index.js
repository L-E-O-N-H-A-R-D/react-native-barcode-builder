...
export default class Barcode extends PureComponent {
  ...
  update() {
    ...
    if (encoded) {
      this.state.bars = this.drawSvgBarCode(encoded, { ...this.props, width: this.props.width / encoded.data.length });
    }
  }

  drawSvgBarCode(encoding, options = {}) {

    options.width = Math.max(options.width, 2.5);
  ...
  render() {
    ...
    return (
      <View style={[styles.svgContainer, backgroundStyle]}>
        <Surface height={this.props.height} width={this.props.width}>
          <Shape d={this.state.bars} fill={this.props.lineColor} />
        </Surface>
        { typeof(this.props.text) != 'undefined' &&
        <Text style={{color: this.props.textColor, width: this.props.width, textAlign: 'center'}} >{this.props.text}</Text>
        }
      </View>
    );
  }
